define(["require", "exports", "react", "./examples/Link.Basic.Example", "./Link.checklist"], function (require, exports, React, Link_Basic_Example_1, Link_checklist_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    var LinkBasicExampleCode = require('!raw-loader!office-ui-fabric-react/src/components/Link/examples/Link.Basic.Example.tsx');
    var LinkBasicExampleCodepen = require('!raw-loader!office-ui-fabric-react/lib/codepen/components/Link/Link.Basic.Example.Codepen.txt');
    exports.LinkPageProps = {
        title: 'Link',
        componentName: 'Link',
        componentUrl: 'https://github.com/OfficeDev/office-ui-fabric-react/tree/master/packages/office-ui-fabric-react/src/components/Link',
        componentStatus: Link_checklist_1.LinkStatus,
        examples: [
            {
                title: 'Link',
                code: LinkBasicExampleCode,
                view: React.createElement(Link_Basic_Example_1.LinkBasicExample, null),
                codepenJS: LinkBasicExampleCodepen
            }
        ],
        propertiesTablesSources: [require('!raw-loader!office-ui-fabric-react/src/components/Link/Link.types.ts')],
        overview: require('!raw-loader!office-ui-fabric-react/src/components/Link/docs/LinkOverview.md'),
        bestPractices: '',
        dos: require('!raw-loader!office-ui-fabric-react/src/components/Link/docs/LinkDos.md'),
        donts: require('!raw-loader!office-ui-fabric-react/src/components/Link/docs/LinkDonts.md'),
        isHeaderVisible: true,
        isFeedbackVisible: true,
        allowNativeProps: true,
        nativePropsElement: ['a', 'button']
    };
});
//# sourceMappingURL=Link.doc.js.map