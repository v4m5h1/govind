define(["require", "exports", "react", "./examples/Label.Basic.Example", "./Label.checklist"], function (require, exports, React, Label_Basic_Example_1, Label_checklist_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    var LabelBasicExampleCode = require('!raw-loader!office-ui-fabric-react/src/components/Label/examples/Label.Basic.Example.tsx');
    exports.LabelPageProps = {
        title: 'Label',
        componentName: 'Label',
        componentUrl: 'https://github.com/OfficeDev/office-ui-fabric-react/tree/master/packages/office-ui-fabric-react/src/components/Label',
        componentStatus: Label_checklist_1.LabelStatus,
        examples: [
            {
                title: 'Label',
                code: LabelBasicExampleCode,
                view: React.createElement(Label_Basic_Example_1.LabelBasicExample, null),
                codepenJS: require('!raw-loader!office-ui-fabric-react/lib/codepen/components/Label/Label.Basic.Example.Codepen.txt')
            }
        ],
        propertiesTablesSources: [require('!raw-loader!office-ui-fabric-react/src/components/Label/Label.types.ts')],
        overview: require('!raw-loader!office-ui-fabric-react/src/components/Label/docs/LabelOverview.md'),
        bestPractices: '',
        dos: require('!raw-loader!office-ui-fabric-react/src/components/Label/docs/LabelDos.md'),
        donts: require('!raw-loader!office-ui-fabric-react/src/components/Label/docs/LabelDonts.md'),
        isHeaderVisible: true,
        isFeedbackVisible: true,
        allowNativeProps: true
    };
});
//# sourceMappingURL=Label.doc.js.map